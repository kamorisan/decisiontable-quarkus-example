...
# Configuration                                                                #
################################################################################
PRJ=("dm-kogito" "Decision Services Demo" "Red Hat Decision Manager Demo")
...
# Kogito Operator のインストール
echo_header "Installing KOGITO Operator"
# Install OpenShift Pipelines Operator
oc apply -f ./operator/kogito-operator.yaml

# Dumb timer. 
# Needs to be replaced by a test on the subscription state:
# oc describe sub openshift-pipelines-operator -n openshift-operators | grep State
runSpinner 10

oc apply -f operator/kogito-deploy.yaml -n $PRJ

### Tekton の リソース のインストール
# oc create -f ./cicd/tekton-resources/ -n $PRJ
# runSpinner 5
# oc expose svc el-ba-cicd-event-listener -n $PRJ


## Front end application の インストール
